...
//retrieve username from jwt token
   ...
   //retrieve expiration date from jwt token
	...
	public Object getRolesFromToken(String token) {	
		return getClaimFromToken(token, Claims::entrySet).stream()
...
//generate token for user
	public String generateToken(UserDetails userDetails) {
		...
		String roles = userDetails.getAuthorities().stream().map(r -> r.getAuthority()).collect(Collectors.joining(","));
		claims.put("usr", userDetails.getUsername());
		claims.put("sub", "Authentication token");
		claims.put("iss", "gateway");
